...
class SwiftPaywall: UIViewController {
    ...
    private func buildSubviews() {
        ...
        let mountains = UIImageView(image: UIImage(named: "Mountains"))
        view.addSubview(mountains)
        mountains.contentMode = .scaleAspectFill
        mountains.eqWidth(view).eqBottom(view).centerX(view).eqHeight(view, 0, 0.15)
...
private class PackageCell : UICollectionViewCell {
    ...
    func setHighlighted( _ highlighted: Bool) {
        if highlighted {
            ...
            monthlyPriceLabel.textColor = UIColor.white.withAlphaComponent(0.7)
        } else {
